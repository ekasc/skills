...
State your read of scope and constraints in one paragraph. Use `ask the user directly` only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
If a phase creates or edits a skill, the phase instructs the implementer to use the **write-a-skill** skill (Cursor's built-in for authoring SKILL.md files).

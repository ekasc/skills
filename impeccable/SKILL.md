---
name: impeccable
description: "Orchestrator for UI/UX improvement work — load when the user wants to design, redesign, or improve a frontend interface (website, landing page, dashboard, product UI, app shell, component, form, settings, onboarding, empty state) and the request spans multiple concerns. Routes to specialist skills: `bolder`, `quieter`, `distill`, `delight`, `polish`, `colorize`, `harden`, `onboard`, `clarify`, `audit`, `critique`, `optimize`, `accessibility-compliance`, `adapt`, `animate`, `extract`, `typeset`, `layout`, `normalize`, `impeccable` family. Prefer loading the specific specialist directly when the request clearly matches one (e.g. 'polish this' → `polish`, 'this is too loud' → `quieter`). Do NOT use for backend-only or non-UI tasks."
---

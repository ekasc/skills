...
**Image-to-code bridge.** When working with images generated by `imagegen-frontend` (or similar image-generation skills), treat those images as visual direction, not as pixel-perfect templates. The images define: composition, hierarchy, density, atmosphere, and distinctive visual moves. Implement them faithfully following the rest of this craft flow. Do not reinterpret them into a different aesthetic; equally, do not attempt to extract exact pixel values or measure precise font sizes from images — approximate the spirit, tune the details during production.

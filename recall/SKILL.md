...
name: recall
description: >-
  Reconstruct your recent working context from your own chat history, live state, and the shared
record (user reports, prior fixes, incidents), then hand back a tight current-state brief. Use for
'recall my work on X', 'catch me up', 'what have I been working on', 'where did I leave off', before
...
> **ad hoc:** invoke only on an explicit user request. Do not auto-trigger or chain from other skills — this skill fans out over chat history and reads transcripts, so it is token-heavy. Keep it on-demand.
...
Transcripts live across your agent tools. In **pi**, session logs are at `~/.pi/agent/sessions/--<encoded-cwd>--/*.jsonl` (project sessions, where `<encoded-cwd>` is the cwd with `/` → `-` and wrapped in `--`, e.g. `/Users/you/proj` becomes `--Users-you-proj--`) and `~/.pi/sessions/*.jsonl` (top-level). **OpenCode** keeps sessions in `~/.local/share/opencode/opencode.db` (SQLite). **Codex** keeps `~/.codex/sessions/2026/**/*.jsonl`. **Claude** keeps `~/.claude/transcripts/*.jsonl`. Every line is one chat message. Read only the in-scope workspace's directory; never glob across unrelated projects.

...
> **ad hoc:** invoke only on an explicit user request. Do not auto-trigger or chain from other skills — it spawns three parallel reviewers, so it is token-heavy. Keep it on-demand.
...
The parent finds its own transcript file before fanning out. Use the active workspace's session directory (`~/.pi/agent/sessions/--<encoded-cwd>--/`). Do not glob across `~/.pi/agent/sessions/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
- Substantive existing-skill edit (a new section, a new pattern table, more than ~10 lines): hand to Cursor's built-in `write-a-skill` skill and run its draft / test / iterate loop.

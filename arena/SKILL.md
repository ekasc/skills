...
> **ad hoc:** invoke only on an explicit user request. Do not auto-trigger or chain from other skills — it generates N full candidates plus a judge, so it is token-heavy. Keep it on-demand.
...
3. Pick the runners. Use `arena runners` from `~/.pi/agent/skills/pstack-models.mdc` when present. Otherwise default to one each on `claude-fable-5-thinking-max`, `gpt-5.6-sol-max`, `grok-4.6-fast-xhigh`, `claude-opus-5-thinking-xhigh`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
